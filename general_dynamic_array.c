...
int main(){
    ...
    // instance of a string dynamic array .

    da *da_strings = da_init(sizeof(char *));

    char *names[6] = {"kartik", "ritik", "naman", "ankur", "honey", "mandeep"};

    for(int i=0; i<6; i++){
        da_append(da_strings, names[i]);
    }

    for(int i=0; i<6; i++){
        char *string = (char *)da_get(da_strings, i);
        printf("%s \n", string);
    }
